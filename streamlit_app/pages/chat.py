...
with st.sidebar:
    ...
    if uploaded_file:
        ...
        session_id = st.session_state.get("jwt_token")
        if not session_id:
            st.warning("Please login before uploading a document.")
        elif file_description:
            if file_key not in st.session_state.uploaded_files:
                # Upload file scoped to this user's session
                success = document_upload_rag(uploaded_file, file_description, session_id)
                if success:
